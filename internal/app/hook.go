...
// lookupBoundTask returns the task whose session_id matches the ambient
// harness's session-id env var (CLAUDE_CODE_SESSION_ID,
// PRAXIS_SESSION_ID, …), or nil if no such task exists, no env var is
// set, or the DB lookup fails. Resolution goes through the same
// currentSessionID/ambientHarness path as `flow show` and
// `flow do --here`, so the hook can never claim a binding those
// commands would deny. Hook code must never fail loud — a hook error
// blocks the user's session — so all errors are swallowed and treated
// as "unbound".
