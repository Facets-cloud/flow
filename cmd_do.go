...
func cmdDo(args []string) int {
	...
	row := tx.QueryRow(`SELECT `+taskCols+` FROM tasks WHERE slug = ?`, task.Slug)
	fresh2, err := scanTask(row)
	if err != nil {
	...
	if task.ProjectSlug.Valid {
		p, err := GetProject(db, task.ProjectSlug.String)
	...
	if needsBootstrap {
		...
		command = fmt.Sprintf("claude %s", shellQuote(prompt))
	...
	if err := SpawnITermTab(buildTabTitle(project, task), cwd, command, envVars); err != nil {
		fmt.Fprintf(os.Stderr, "error: %v\n", err)
		return 1
	}
...
func buildBootstrapPrompt(slug string) string {
	return fmt.Sprintf(
		"You are the execution session for flow task %s. Do ALL of the following in order before touching code:\n"+
			"1. Run: flow register-session  (no args, uses FLOW_TASK env var). This records your session_id so future flow do calls can resume this session.\n"+
...
func buildTabTitle(project *Project, task *Task) string {
	raw := task.Slug
	if project != nil {
...
func findTask(db *sql.DB, query string) (*Task, int) {
