...
func TestCmdAddProjectHappyPath(t *testing.T) {
	...
	db := openFlowDB(t)
	p, err := GetProject(db, "auth-service")
	if err != nil {
	...
	if _, err := GetWorkdir(db, wd); err != nil {
...
func TestCmdAddProjectCollisionAvoidance(t *testing.T) {
	...
	db := openFlowDB(t)
	if _, err := GetProject(db, "same-name"); err != nil {
		t.Errorf("same-name missing: %v", err)
	}
	if _, err := GetProject(db, "same-name-2"); err != nil {
...
func TestCmdAddTaskInheritsProjectWorkDir(t *testing.T) {
	...
	db := openFlowDB(t)
	task, err := GetTask(db, "child-task")
	if err != nil {
		t.Fatal(err)
	}
	if task.WorkDir != wd {
