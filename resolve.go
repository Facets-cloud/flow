...
func ResolveTask(db *sql.DB, ref string, includeArchived bool) (*Task, error) {
	ref = strings.TrimSpace(ref)
	if ref == "" {
		return nil, fmt.Errorf("empty task ref")
	}

	t, err := GetTask(db, ref)
	if err != nil {
...
func ResolveProject(db *sql.DB, ref string, includeArchived bool) (*Project, error) {
	ref = strings.TrimSpace(ref)
	if ref == "" {
		return nil, fmt.Errorf("empty project ref")
	}

	p, err := GetProject(db, ref)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
