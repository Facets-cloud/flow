...
func TestCmdListTasksArchivedHiddenByDefault(t *testing.T) {
	...
	if _, err := db.Exec(`UPDATE tasks SET archived_at = ? WHERE slug = ?`, nowISO(), "dead"); err != nil {
		t.Fatal(err)
	}
	out := captureStdout(t, func() {
		if rc := cmdList([]string{"tasks"}); rc != 0 {
...
func TestCmdListProjectsArchivedHidden(t *testing.T) {
	...
	if _, err := db.Exec(`UPDATE projects SET archived_at = ? WHERE slug = ?`, nowISO(), "gone"); err != nil {
		t.Fatal(err)
	}
	out := captureStdout(t, func() {
		if rc := cmdList([]string{"projects"}); rc != 0 {
